...
#--# Query Against Database for Abundances origin-----------------------------------------------------
def DBAbundancesQuery(PoolName):
  return pd.read_sql_query('select rs.sample_id, m.c_muestra_wineseq, tm.d_tipo_muestra, rs.name, r.name, c.ftp_path, ct.name'
                              ' from run_samples   rs'
                              ' join runs           r on rs.run_id = r.id'
                              ' join muestra        m on m.id = rs.sample_id'
                              ' join tipo_muestra  tm on tm.c_tipo_muestra = m.c_tipo_muestra'
                              ...
                              #' and c.ftp_path != \'JacksonFamWine\''
                              ' and c.ftp_path != \'upload_errors\''
                              ' and r.name !=  \'upload_errors\'',DBconexion())

#--# @uery to know finished samples for client and sampletype----------------------------------------- 
